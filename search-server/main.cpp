...
public:
        ...
        : stop_words_(MakeUniqueNonEmptyStrings(stop_words))
    {
        if (all_of(stop_words_.begin(), stop_words_.end(),
                   [](const string &word){return IsValidWord(word);}) == false)
        {
                     ...
                     DocumentStatus status, const vector<int>& ratings)
    {
        ...
        queue_added_documents_.push_back(document_id);
    }
    ...
    int GetDocumentId(int index) const
    {
        if (index < 0)
        {
            throw out_of_range("Индекс переданного документа выходит за "
        ...
        auto it = queue_added_documents_.begin();
        advance(it, index);
        return *it.base();
    }
    
    static bool is_first_symbol_a_minus(const string &str)
    {
        return str[0] == '-';
    }
...
private:
    ...
    vector<int> queue_added_documents_;
    ...
    QueryWord ParseQueryWord(string text) const
    {
        ...
        if (is_first_symbol_a_minus(text))
        {
            is_minus = true;
            text = text.substr(1);
        }
...
int main()
{
    ...
    //    const set<string> stop_words = {"\tи"s, "в"s, "на"s};
    //    SearchServer search_server2(stop_words);
    ...
    search_server.AddDocument(5, "большой --пёс"s,
                              DocumentStatus::ACTUAL, {1, 3, 2});
    
    
    search_server.AddDocument(6, "большой -пёс"s,
                              DocumentStatus::ACTUAL, {1, 3, 2});
    ...
    search_server.AddDocument(7, "\t большой-пёс"s,
                              DocumentStatus::ACTUAL, {1, 3, 2});
    
    
    search_server.AddDocument(8, "большой--пёс"s,
                              DocumentStatus::ACTUAL, {1, 3, 2});
