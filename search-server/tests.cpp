...
void TestRemoveDuplicates()
{
    ...
    ASSERT_HINT(search_server.GetDocumentCount() == 9,
                "Некорретно работает метод добавления документов");
    RemoveDuplicates(search_server);
    ASSERT_HINT(search_server.GetDocumentCount() == 5,
                "Некорректно удаляются дупликаты документов");
}
